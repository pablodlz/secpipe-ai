...
def test_export_rejects_http_remote(monkeypatch) -> None:
    monkeypatch.setenv("DEFECTDOJO_URL", "http://dojo.remote.example")   # http host remoto -> recusa (token cleartext)
    monkeypatch.setenv("DEFECTDOJO_TOKEN", "t")
    res = DefectDojoExporter(DefectDojoConfig()).export(_report())
    assert not res.ok and "https" in res.detail
...
def test_export_posts_sarif_with_auth(monkeypatch) -> None:
    ...
    monkeypatch.setattr(dd, "open_no_redirect", fake_open)
    res = DefectDojoExporter(DefectDojoConfig()).export(_report())
...
def test_http_error_redacts_token(monkeypatch) -> None:
    ...
    monkeypatch.setattr(dd, "open_no_redirect", boom)
    res = DefectDojoExporter(DefectDojoConfig()).export(_report())
...
def test_reimport_uses_reimport_endpoint(monkeypatch) -> None:
    ...
    monkeypatch.setenv("DEFECTDOJO_TOKEN", "t")
    captured: dict[str, object] = {}
    monkeypatch.setattr(dd, "open_no_redirect",
